...
(function () {
    ...
    const ALLOWED_TYPES = {
        ...
        QUYEN_COMBOGRID_CLICK_RESULT: true,
        QUYEN_CONFIG: true,
        QUYEN_DRUG_LIST_RESULT: true,
        QUYEN_FILL_COMBOGRID: true,
        QUYEN_IFRAME_DRUGS: true,
        QUYEN_KEYBOARD_SELECT: true,
        QUYEN_KEYBOARD_SELECT_RESULT: true,
        QUYEN_PATIENT_INFO_RESULT: true,
        QUYEN_PATIENT_SELECTED: true,
        QUYEN_REQ_CALL_SP: true,
        QUYEN_REQ_CARESHEET_SEC4: true,
        QUYEN_REQ_DRUG_LIST: true,
        QUYEN_REQ_PATIENT_INFO: true,
        QUYEN_REQ_VATTU_DATA: true,
        QUYEN_REQ_VITALS: true,
        QUYEN_SP_RESULT: true,
        QUYEN_TRIGGER_CHANGE: true,
        QUYEN_TRIGGER_SEARCH: true,
        QUYEN_TYPE_TEXT: true,
        QUYEN_VITALS_RESULT: true,
        QUYEN_WAKE_UP_GRID: true,
        QUYEN_FILL_ERROR: true,
        QUYEN_FORM_PATIENT_CONTEXT: true,
        QUYEN_FORM_FOCUSED: true,
        QUYEN_FORM_CLOSED: true,
        QUYEN_VATTU_DATA_RESULT: true,
        QUYEN_FILL_VT_ITEM: true,
        QUYEN_VT_FILL_RESULT: true,
        QUYEN_VT_SEND_ENTER: true,
        QUYEN_VT_ENTER_RESULT: true,
        QUYEN_VT_PHYSICAL_ENTER_PRESSED: true,
        QUYEN_ROLE_BLOCK: true,
        QUYEN_HIS_ENV: true
    };
    ...
    function redactLogValue(value) {
        ...
        if (typeof value === 'string') {
            return value
                .replace(/\b\d{1,2}\/\d{1,2}\/\d{4}\b/g, '[DOB]')
                .replace(/\b\d{6,}\b/g, '[ID]')
                .replace(/\b[A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+(?:\s+[A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+){1,5}\b/g, '[NAME]');
        }
        if (typeof value === 'object') {
            if (Array.isArray(value)) return value.map(redactLogValue);
            const out = {};
            Object.keys(value).forEach(function (key) {
                if (/name|hoten|ngaysinh|dob|khambenh|hosobenh|benhnhan|token|password/i.test(key)) {
                    out[key] = '[REDACTED]';
                } else {
    ...
    function safeArgs(args) {
        return Array.prototype.slice.call(args || []).map(redactLogValue);
    }

    const log = {
        debug: (...args) => { if (BRIDGE_DEBUG) console.log('[__EXT_EMOJI__ Bridge]', ...safeArgs(args)); },
        warn: (...args) => console.warn('[__EXT_EMOJI__ Bridge]', ...safeArgs(args)),
        error: (...args) => console.error('[__EXT_EMOJI__ Bridge]', ...safeArgs(args))
    };

    function getBridgePatientContext() {
        return {
            seq: _patientSeq || 0,
            khambenhId: _currentKhambenhId || '',
            hosobenhanid: _currentHosobenhanid || ''
        };
    }

    function postToContent(type, payload) {
        if (!ALLOWED_TYPES[type]) {
            log.warn('Blocked postToContent unknown type:', type);
            return false;
        }
        payload = payload || {};
    ...
    let _lastSelectedRowId = '';
    let _vtFillInProgress = false; // ★ Guard: Chặn chuyển BN khi đang điền VT

    // ★ SAFETY v2: Sequence counter + Patient ID tracking
    // Mỗi lần chọn BN mới → _patientSeq++ → mọi async response phải khớp seq
    // Patient state initialized near the top because bridge-ready messages can be posted before hooks run.
    ...
    function onPatientSelected(rowId) {
        // ★ Guard: Chặn chuyển BN khi đang điền VT — events mô phỏng trong form
        // có thể bubble lên grid, gây ra re-select row khác
        if (_vtFillInProgress) {
            log.debug('👤 Skip onPatientSelected — VT fill đang chạy');
            return;
        }
        ...
        function continueVitalsFlow() {
            ...
            function finishVitals() {
                ...
                postToContent('QUYEN_PATIENT_SELECTED', {
                    seq: thisSeq,
                    patient: {
                        name: hoTen,
                        dob: ngaySinh,
    ...
    function checkForCareSheetData(options, data) {
        ...
        if (hasSec4 || hasSec17 || weight) {
            ...
            postToContent('QUYEN_FORM_FOCUSED', { tab: 'caresheet' });

            postToContent('QUYEN_CARESHEET_SEC4_DATA', {
                seq: snoopSeq,
                khambenhId: snoopKB,
                data: sec4,
                sec17: sec17,
    ...
    function checkForDrugData(options, data) {
        ...
        if (urlStr.includes('NTU02D007') || urlStr.includes('PhieuTruyenDich')) {
            ...
            if (rows.length > 0) {
                ...
                postToContent('QUYEN_FORM_FOCUSED', { tab: 'infusion' });
    ...
    function handleCareSheetSec4Request(seq, requestedKhambenhId, requestId) {
        ...
        if (phieuIds.length === 0) {
            ...
            postToContent('QUYEN_CARESHEET_SEC4_DATA', { seq: reqSeq, requestId: requestId, khambenhId: _currentKhambenhId, data: {} });
            return;
        }
    ...
    function fetchVitalsFromHIS() {
        if (!_jsonrpc) {
            ...
            postToContent('QUYEN_VITALS_RESULT', { vitals: {} });
            return;
        }
        ...
        postToContent('QUYEN_VITALS_RESULT', { vitals: vitals });
    }
    ...
    function fetchVatTuData(khambenhId, benhnhanId, hosobenhanid, requestId) {
        if (!khambenhId) {
            ...
            postToContent('QUYEN_VATTU_DATA_RESULT', { requestId: requestId, drugs: [], existingVT: [] });
            return;
        }
        ...
        function onBothDone() {
            ...
            function finalize() {
                ...
                postToContent('QUYEN_VATTU_DATA_RESULT', {
                    requestId:  requestId,
                    drugs:      allDrugs,
    ...
    // ★ Architecture v4:
    //   1. Poll chờ #cboMA_KHO có giá trị (widget sẵn sàng)
    //   2. Delay 1.5s cho ComboGrid tải data xong
    //   3. Native KeyboardEvent gõ từng ký tự
    //   4. Poll cg-* popup items (NO jqgrow fallback)
    // ==========================================
    function handleFillVtItem(data) {
        ...
        // ── 1. Tìm form VT (chỉ lấy element ĐANG HIỂN THỊ) ──
        var allDocs = getAllDocuments();
        var vtDoc = null, vtWin = null;
        for (var di = 0; di < allDocs.length; di++) {
            try {
                var _el = allDocs[di].getElementById('txtDS_THUOC');
                if (_el) {
                    try { var _r = _el.getBoundingClientRect(); if (_r.width < 5) continue; }
                    catch(e3) { continue; }
        ...
        if (!vtInput) { postResult(false, 'Không tìm thấy #txtDS_THUOC.'); return; }

        var elWin = vtWin || (vtInput.ownerDocument && vtInput.ownerDocument.defaultView) || window;
        var $jq = (vtWin && (vtWin.jQuery || vtWin.$)) || null;
        ...
        function fillDetailsAndSave(fallbackDelay) {
            ...
            function doFill() {
                ...
                var verifyErrors = [];
                if (slInput && String(slInput.value || '').trim() !== String(sl)) verifyErrors.push('số lượng');
                if (vtInput && ten && String(vtInput.value || '').trim().length < 2) verifyErrors.push('vật tư');
                if (verifyErrors.length > 0) {
                    postResult(false, 'Không xác minh được trường: ' + verifyErrors.join(', '));
                    return;
                }
                ...
                if (addBtn) {
                    ...
                    addBtn.click();
                    postToContent('QUYEN_VT_PHYSICAL_ENTER_PRESSED', { ma: ma });
                } else {
         ...
         function startTyping() {
                ...
                try {
                    ...
                    if (!widget) {
                        ...
                        for (var ki = 0; ki < dataKeys.length; ki++) {
                            var v = allData[dataKeys[ki]];
                            if (v && typeof v === 'object' && typeof v.search === 'function') {
                                widget = v; break;
                            }
                        }
                    }
            ...
            if (widget && fastPathEnabled) {
                ...
                log.debug('🧰 Fast Path đang tắt theo policy rollout, dùng luồng gõ/click.');
            }
            ...
            function typeNextChar() {
                if (charIdx >= searchTerm.length) {
                    try {
                        vtInput.dispatchEvent(new elWin.KeyboardEvent('keydown', {
                            keyCode: 40, which: 40, key: 'ArrowDown', bubbles: true, cancelable: true
                        }));
                    ...
                    log.debug('🧰 Gõ xong + ArrowDown, chờ combogrid...');
                    startComboGridPolling();
                    return;
                }
                ...
                try {
                    vtInput.dispatchEvent(new elWin.KeyboardEvent('keydown', { keyCode: kc, which: kc, key: char, bubbles: true, cancelable: true }));
                    vtInput.dispatchEvent(new elWin.KeyboardEvent('keypress', { keyCode: kc, which: kc, key: char, bubbles: true, cancelable: true }));
                    vtInput.value = vtInput.value + char;
                    vtInput.dispatchEvent(new elWin.Event('input', { bubbles: true }));
                    vtInput.dispatchEvent(new elWin.KeyboardEvent('keyup', { keyCode: kc, which: kc, key: char, bubbles: true, cancelable: true }));
                } catch(e) {
                    vtInput.value = vtInput.value + char;
            ...
            retriggerTimer = setInterval(function() {
                ...
                if (retriggerCount > 1 && retriggerCount % 3 === 0) {
                    ...
                    function retypeChar() {
                        ...
                        try {
                            vtInput.dispatchEvent(new elWin.KeyboardEvent('keydown', { keyCode: k2, which: k2, key: c2, bubbles: true }));
                            vtInput.value = vtInput.value + c2;
                            vtInput.dispatchEvent(new elWin.Event('input', { bubbles: true }));
                            vtInput.dispatchEvent(new elWin.KeyboardEvent('keyup', { keyCode: k2, which: k2, key: c2, bubbles: true }));
                        } catch(e3) { vtInput.value = vtInput.value + c2; }
                        ri++;
                        setTimeout(retypeChar, 50);
                    }
                    retypeChar();
                }
            }, 2000);
    ...
    // ==========================================
    // SEND ENTER TO VT FORM — Click nút "Thêm vật tư"
    // Synthetic keyboard Enter không hoạt động với HIS
    // → tìm và click trực tiếp nút "Thêm vật tư"
    // ==========================================
    function handleVtSendEnter() {
        var allDocs = getAllDocuments();
        var vtDoc = null, vtWin = null;
        for (var di = 0; di < allDocs.length; di++) {
            try {
        ...
        if (!vtDoc) {
            log.warn('🧰 handleVtSendEnter: Không tìm thấy phiếu VT');
            postToContent('QUYEN_VT_ENTER_RESULT', { success: false, error: 'Chưa mở phiếu VT' });
            return;
        }

        // ── Tìm nút "Thêm vật tư" trong form ──
        var addBtn = null;
        var allBtns = vtDoc.querySelectorAll('button, input[type="button"], a.btn, .btn');
        for (var bi = 0; bi < allBtns.length; bi++) {
            var btnText = (allBtns[bi].textContent || allBtns[bi].value || '').trim();
            if (btnText.indexOf('Thêm vật tư') >= 0 || btnText.indexOf('Thêm VT') >= 0 || btnText.indexOf('ThemVT') >= 0) {
                addBtn = allBtns[bi];
                break;
            }
        }

        // Fallback: tìm theo ID phổ biến
        if (!addBtn) {
            addBtn = vtDoc.getElementById('btnThemVT') || vtDoc.getElementById('btnThem') || vtDoc.getElementById('btnAdd');
        }

        if (addBtn) {
            log.debug('🧰 Click nút "Thêm vật tư":', addBtn.textContent || addBtn.id);
            addBtn.click();
            postToContent('QUYEN_VT_ENTER_RESULT', { success: true });
            return;
        }

        // Fallback cuối: thử gọi hàm toàn cục HIS nếu có
        if (vtWin) {
            var fnNames = ['addVatTu', 'themVatTu', 'ThemVatTu', 'AddRow', 'addRow', 'saveRow'];
            for (var fi = 0; fi < fnNames.length; fi++) {
                if (typeof vtWin[fnNames[fi]] === 'function') {
                    log.debug('🧰 Gọi hàm HIS:', fnNames[fi]);
                    try { vtWin[fnNames[fi]](); } catch (e) { log.warn('🧰 Lỗi gọi', fnNames[fi], e.message); }
                    postToContent('QUYEN_VT_ENTER_RESULT', { success: true });
                    return;
                }
            }
        ...
        log.warn('🧰 Không tìm được nút "Thêm vật tư" trong form');
        postToContent('QUYEN_VT_ENTER_RESULT', { success: false, error: 'Không tìm thấy nút Thêm vật tư' });
    }
    ...
    function broadcastDrugs(drugs) {
        postToContent('QUYEN_DRUG_LIST_RESULT', {
            drugs: drugs,
            count: drugs.length,
            source: 'auto-capture'
        });
    ...
    // ==========================================
    function handleTypeText(inputMarker, text, directEl) {
        try {
            const el = directEl || findMarkedElement('data-quyen-input', inputMarker);
            if (!el) {
    ...
    function callSP(spName, params, requestId) {
        const ALLOWED_SP_CALLS = {
            'NT.006': true,
            'NTU02D204.06': true
        };
        try {
            if (!_jsonrpc) { sendResult('QUYEN_SP_RESULT', { error: 'no jsonrpc' }, requestId); return; }
            if (!ALLOWED_SP_CALLS[spName]) {
                sendResult('QUYEN_SP_RESULT', { error: 'SP_NOT_ALLOWED', spName: spName }, requestId);
                return;
            }
    ...
    function sendResult(type, data, requestId) {
        postToContent(type, Object.assign({}, data || {}, { requestId: requestId }));
    }
    ...
    (function watchFormFocus() {
        ...
        function scanForms() {
            // Check VT form dialog (mở dạng popup trong trang, không phải iframe)
            var allDocs = getAllDocuments();
            for (var ddi = 0; ddi < allDocs.length; ddi++) {
                try {
                    var vtInput = allDocs[ddi].getElementById('txtDS_THUOC');
                    if (vtInput) {
                        // Kiểm tra form đang hiển thị (không bị ẩn)
                        try { var vtRect = vtInput.getBoundingClientRect(); if (vtRect.width < 5) continue; }
                        catch(e3) { continue; }
                        if (_lastTab !== 'vattu') {
                            _lastTab = 'vattu';
                            postToContent('QUYEN_FORM_FOCUSED', { tab: 'vattu' });

                            // ★ Auto-fill BS kê đơn + chọn Kho VT khi mở form
                            var vtFormDoc = allDocs[ddi];
                            var vtFormWin = vtFormDoc.defaultView;
                            setTimeout(function() {
                                try {
                                    // 1. Điền tên bác sĩ kê đơn
                                    var bsField = vtFormDoc.getElementById('txtTKBACSIID');
                                    if (bsField && !(bsField.value || '').trim()) {
                                        // Lấy tên BS từ drugs đã capture
                                        var doctorName = '';
                                        if (_capturedDrugs && _capturedDrugs.length > 0) {
                                            for (var dri = 0; dri < _capturedDrugs.length; dri++) {
                                                if (_capturedDrugs[dri].doctor) { doctorName = _capturedDrugs[dri].doctor; break; }
                                            }
                                        }
                                        if (doctorName) {
                                            bsField.value = doctorName;
                                            if (vtFormWin && vtFormWin.$) {
                                                vtFormWin.$(bsField).val(doctorName).trigger('input').trigger('keyup').trigger('change');
                                            } else {
                                                bsField.dispatchEvent(new Event('input', { bubbles: true }));
                                                bsField.dispatchEvent(new Event('change', { bubbles: true }));
                                            }
                                            log.debug('🧰 Auto-fill BS kê đơn:', doctorName);
                                        }
                                    }

                                    // 2. Chờ popup đóng và chọn tủ trực
                                    function doAutoClose() {
                                        var mainDoc = vtFormDoc.defaultView ? vtFormDoc.defaultView.parent.document : document;
                                        var jboxes = mainDoc.querySelectorAll('.jBox-wrapper');
                                        for (var ji = 0; ji < jboxes.length; ji++) {
                                            var titleEl = jboxes[ji].querySelector('.jBox-title div:first-child');
                                            if (titleEl && (titleEl.textContent || '').indexOf('Thuốc thường dùng') >= 0) {
                                                var closeBtn = jboxes[ji].querySelector('.jBox-closeButton');
                                                if (closeBtn) { closeBtn.click(); log.debug('🧰 Auto-đóng popup "Thuốc thường dùng"'); }
                                                var msgBtns = mainDoc.querySelectorAll('.jBox-wrapper button');
                                                for (var mi = 0; mi < msgBtns.length; mi++) {
                                                    if ((msgBtns[mi].textContent || '').indexOf('Đồng ý') >= 0) {
                                                        msgBtns[mi].click(); log.debug('🧰 Auto-đóng "Đồng ý"');
                                                    }
                                                }
                                                return;
                                            }
                                    ...
                                    var khoAttempt = 0;
                                    var khoTimer = setInterval(function() {
                                        khoAttempt++;
                                        var khoSelect = vtFormDoc.getElementById('cboMA_KHO');
                                        if (khoSelect && (!khoSelect.value || khoSelect.value === '' || khoSelect.value === '-1' || khoSelect.value === '0')) {
                                            for (var ki = 0; ki < khoSelect.options.length; ki++) {
                                                ...
                                                if (optVal.indexOf('4251') >= 0 || optText.indexOf('Tủ trực VTYT') >= 0) {
                                                    ...
                                                    if (vtFormWin && vtFormWin.$) {
                                                        ...
                                                        khoSelect.dispatchEvent(new Event('change', { bubbles: true }));
                                                    }
                                                    log.debug('🧰 Auto-chọn Kho VT [Attempt ' + khoAttempt + ']: ' + optText + ' (' + optVal + ')');
                                                    setTimeout(doAutoClose, 500);
                                                    setTimeout(doAutoClose, 1000);
                                                    setTimeout(doAutoClose, 2000);
                                        ...
                                        if (khoAttempt > 15) clearInterval(khoTimer);
                                    }, 500);
                                } catch(e5) { log.warn('🧰 Auto-fill VT form error:', e5); }
                            }, 800);
                        }
                        return;
                    }
                } catch(e4) { /* cross-origin */ }
            }
            ...
            for (let i = 0; i < level1.length; i++) {
                try {
                    ...
                    if (tab) {
                        if (_lastTab !== tab) {
                            _lastTab = tab;
                            postToContent('QUYEN_FORM_FOCUSED', { tab: tab });
                        }
                        return;
                    }
                    ...
                    for (let j = 0; j < level2.length; j++) {
                        try {
                            ...
                            if (tab2) {
                                if (_lastTab !== tab2) {
                                    _lastTab = tab2;
                                    postToContent('QUYEN_FORM_FOCUSED', { tab: tab2 });
                                }
                                return;
                            }
                        } catch (e) { if (e && e.message) console.debug("[Bridge] catch:", e.message); }
            ...
            if (_lastTab !== null) {
                _lastTab = null;
                postToContent('QUYEN_FORM_CLOSED', {});
            }
        }
